...
describe("FamilyScreen", () => {
        ...
        rides: [
          {
            ...
            vehicleId: "v1",
            vehicleLabel: "Van",
            passengerRequestIds: ["ask-apollo"],
            status: "ACTIVE",
          },
        ],
      },
  ...
  describe("ride revert undo", () => {
    ...
    it("cancels a PENDING team ask via RevertRideLink without a dialog", async () => {
      ...
      await waitFor(() => {
        expect(cancelRide).toHaveBeenCalledWith("tok", "s1", "fulfill-ask")
      })
    })
    ...
    it("shows Undo after withdraw and re-accepts on click", async () => {
      ...
      const withdrawRide = vi.fn().mockResolvedValue({
        id: "fulfill-in",
        spaceId: "s1",
        eventKey: "UID:withdraw",
        leg: "TO",
        ...
        vehicleId: "v1",
        vehicleLabel: "SUV",
        passengerRequestIds: ["ask-in"],
        status: "WITHDRAWN",
      })
      const acceptRide = vi.fn().mockResolvedValue([
        {
          id: "fulfill-in",
          spaceId: "s1",
          eventKey: "UID:withdraw",
      ...
      await waitFor(() => {
        expect(withdrawRide).toHaveBeenCalledWith("tok", "s1", "fulfill-in")
      })
    ...
    it("clears auto-declined session id on Reconsider accept", async () => {
      ...
      const coveredCalendarItem = calendarItem({
        id: "e-reconsider",
        source: "FEED",
        title: "Practice",
        startsAt: "2030-08-15T17:00:00.000Z",
      ...
      const assignCalendarCoverage = vi.fn().mockResolvedValue(coveredCalendarItem)
      ...
      await waitFor(() => {
        expect(assignCalendarCoverage).toHaveBeenCalledWith("tok", "FEED", "e-reconsider", {
          coveringAdultId: "1",
          kidIds: ["k1"],
        })
      })
    ...
    it("cancels an ACCEPTED teammate ride via RevertRideLink", async () => {
      ...
      const cancelRide = vi.fn().mockResolvedValue({
        ...
        vehicleId: "v9",
        vehicleLabel: "Van",
        passengerRequestIds: ["ride-teammate"],
        status: "CANCELLED",
      })
      const acceptedOwn = {
        id: "ride-teammate",
      ...
      await waitFor(() => {
        expect(cancelRide).toHaveBeenCalledWith("tok", "s1", "fulfill-teammate")
      })
    })
