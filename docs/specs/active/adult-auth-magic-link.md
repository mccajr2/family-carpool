...
- [ ] OpenAPI documents request-code, verify-code, current-adult, and logout
      (or equivalent names), plus a Bearer HTTP security scheme; **no** greeting
      path remains.
- [ ] Requesting a code for a valid email returns a success response that does
      **not** include the plaintext code in production-shaped config; under the
      ...
      real SMTP.
- [ ] Verifying a correct, unexpired, unused code creates an adult on first use
      and returns a Bearer token; repeating verify with the same code fails.
- [ ] Invalid / expired / rate-limited request or verify fails with documented
...
- [ ] `GET` current adult with a valid Bearer returns that adult; without /
      with revoked token returns 401.
- [ ] Logout revokes the session; the old Bearer then gets 401.
- [ ] Web, Android, and iOS each expose a minimal email+code sign-in flow and
      can show signed-in identity and sign out; tokens stored per Approach.
- [ ] Greeting harness is gone: no `backend/modules/greeting/`, no greeting API
      clients/UI, no greeting OpenAPI path; README quick start documents auth
      smoke instead.
- [ ] Backend unit + integration tests cover code lifecycle, auto-create,
      session auth, logout, and rate-limit or lockout behavior enough that
      reverting the logic fails a test.
- [ ] Client tests (web component/unit; `sharedLogic` host tests) cover auth API
      client + session handling enough that reverting fails a test.
- [ ] `ModularityTests` still passes with the new module (and without greeting).

## Tasks

- [ ] **Contract:** Replace greeting OpenAPI with auth paths + Bearer scheme
      (remove `/api/greeting`).
- [ ] **Backend:** Postgres (or agreed durable store) + migrations; `auth`
      ...
      matching OpenAPI.
- [ ] **Backend:** Delete `backend/modules/greeting/` and its tests.
- [ ] **Web:** Remove greeting client/harness; auth API client, session holder,
      minimal sign-in / me / sign-out UI.
- [ ] **Mobile (`sharedLogic`):** Remove greeting client; auth client + secure
      token storage API used by both apps.
- [ ] **Android:** Remove greeting UI; minimal sign-in / me / sign-out wired to
      sharedLogic.
- [ ] **iOS:** Remove greeting UI; minimal sign-in / me / sign-out wired to
      sharedLogic.
- [ ] **Tests:** Backend unit + integration; web tests; sharedLogic host tests;
      ModularityTests green; no obsolete greeting tests left.
- [ ] **Docs:** Auth decisions in `docs/architecture.md`; README quick start →
