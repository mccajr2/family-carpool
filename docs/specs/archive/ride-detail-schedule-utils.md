...
Feeds: [`ride-detail-shell`](../active/ride-detail-shell.md) →
...
- Detail screen UI, Agenda/`canRoute` entry points, or Route/Playlist chrome
  ([`ride-detail-shell`](../active/ride-detail-shell.md) and later)
...
- Choosing `bufferMinutes` product defaults (mockup 45 game / 15 practice vs
  ...
  [`ride-route-tab`](../planned/ride-route-tab.md); this slice only consumes a
...
- Downstream stubs (read Problem/Notes only if needed for type naming):
  [`ride-detail-shell`](../active/ride-detail-shell.md),
  [`ride-route-tab`](../planned/ride-route-tab.md),
  [`ride-playlist-tab`](../planned/ride-playlist-tab.md)
