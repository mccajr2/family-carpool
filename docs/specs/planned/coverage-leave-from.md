...
Leave-from (and thus leave-by) is too coarse for real mornings. Adults need a
clear choice of **default named place**, **another named place**, or a
**one-time** origin for this coverage/event — and when two adults take
**separate cars** and **separate kids** to the same event, leave-from / leave-by
must match **who is covering whom**, not a single shared origin on the event.
...
- Builds on shipped leave-by + coverage. Origin order today: item override →
  per-adult default → first located place — `/spec` should add **default vs
  other named place vs one-time** (and a **coverage-level** leave-from on the
  responsibility row) with sensible fallbacks for uncovered kids / before assign.
...
- Prefer one vertical PR: API + leave-by computation + Agenda (web). Expo later.
