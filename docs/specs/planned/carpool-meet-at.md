...
Updated: 2026-09-13 · point archived leg-split; kid-split still defers locations here
...
([`carpool-leg-split-plans`](../archive/carpool-leg-split-plans.md) shipped
drivers + shared leave-from only;
[`carpool-kid-split-plans`](../active/carpool-kid-split-plans.md) reuses that
shared leave-from for per-kid plans and still does **not** own places). Getting
there vs Coming back may use different **Picking up from** / **Dropping off at**
saved places.
...
- Progressive **per-kid** plans (shared leave-from only) —
  [`carpool-kid-split-plans`](../active/carpool-kid-split-plans.md)
...
- **Owns true per-leg location persistence** (OpenAPI + leg-slot places;
  independent location fields in the split / kid-split editors). Deferred here
  from `/spec carpool-leg-split-plans` (2026-09-11) and kept here through
  `/spec carpool-kid-split-plans` (2026-09-13) so driver + kid-split + queue can
  ship without a locations mega-PR. If `/spec` reveals locations + meet-at are
  too large together, **split** then — do not invent a third id preemptively.
...
- Drop-off at the driver’s house still needs an agreed time if it is not the
  ...
  slice.
- Ranked after legs + coverage-card + split-plans + kid-split (2026-09-10
  re-rank split; kid-split promoted 2026-09-13) — web first.
