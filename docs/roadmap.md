...
| Agenda week strip        | **Five-day coverage/status** rail in Calendar Context (today + next four local days; event counts, not kids; no driver copy). Not the month/week grid (`family-calendar-grid`). Done: `[agenda-week-glance](specs/archive/agenda-week-glance.md)`. KMP mobile port cancelled; Expo port carved later if needed. Confirmed-ride **Route** detail (ordered stops, maps deep link, leave-by with pickups): Done `[ride-detail-shell](specs/archive/ride-detail-shell.md)` → Done `[ride-route-tab](specs/archive/ride-route-tab.md)` (supersedes cancelled `carpool-multi-stop`) → Done `[ride-playlist-tab](specs/archive/ride-playlist-tab.md)` (Spotify shipped, then **parked** — see Carpool music). Hide Playlist chrome via `[ride-detail-route-only](specs/active/ride-detail-route-only.md)`; Route polish: `[ride-detail-polish](specs/planned/ride-detail-polish.md)`. Mockup SoT: `[docs/ui-system/carpool-combined-flow.jsx](ui-system/carpool-combined-flow.jsx)`. |
...
| ---- | ------------------------------- | ------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 1    | ride-detail-route-only          | active  | 2026-09-08 · enhancement   | Hide Playlist tab chrome on confirmed-ride detail; Route-only surface (Spotify code stays dormant)                            |
| 2    | ride-detail-polish              | planned | 2026-09-06 · enhancement   | Route loading / notify errors / explicit OSRM-unreachable leave-by (playlist chrome parked)                                   |
...
| 6    | push-notifications              | planned | 2026-08-07 · initial       | Expo push for carpool ride **request / accept / deny (pass)** — also backs Route notify call sites when ready                 |
...
| 9    | event-arrival-lead-time         | planned | 2026-08-11 · enhancement   | Editable arrival lead times; reconcile Agenda + Route `bufferMinutes` (Route interim lock: 45/20/0 via `ride-route-tab`)     |
...
| Id                     | Branch                   | Spec                                                |
| ---------------------- | ------------------------ | --------------------------------------------------- |
| ride-detail-route-only | `ride-detail-route-only` | [active](specs/active/ride-detail-route-only.md)    |
