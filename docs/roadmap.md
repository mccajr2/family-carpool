...
| Coverage                 | **Responsibility** rows (adult + kid subset + PENDING/CONFIRMED/DECLINED); any member assigns; assignee confirms/declines; kid exclusive per item on active rows; not a trip/seat plan. API `uncoveredKidIds` stays orthogonal to rides until later ranks wire lifecycle coupling. **Hero & coverage redesign (2026-08-28):** shared priority queue Done: [`coverage-priority-engine`](specs/archive/coverage-priority-engine.md); Done: [`ride-revert-undo`](specs/archive/ride-revert-undo.md); next [`auto-decline-unofferable`](specs/planned/auto-decline-unofferable.md) supersede `[assign-cancels-carpool-request](specs/planned/assign-cancels-carpool-request.md)` for ride/coverage coupling. Done: `[coverage-confirm-decline](specs/archive/coverage-confirm-decline.md)` |
| Event RSVP               | **Hero & coverage redesign (2026-08-28):** per kid + event attendance defaults **going**; **not going** is manual-only, never a hero item, no "not sure" state — [ADR-0003](decisions/ADR-0003-attendance-manual-default-going.md), [`attendance-manual-toggle`](specs/active/attendance-manual-toggle.md). Assigning a real driver resets going. **This slice:** replace Agenda three-way RSVP with the manual toggle; existing API `YES`/`NO`/`NO_RESPONSE` remains; client maps `YES`/`NO_RESPONSE` → going, `NO` → not_going (OpenAPI rename deferred). Out of play when every kid is not going. Copy uses **"going"** / **"not going"** (never ride-side **"drive"** language). Supersedes three-way RSVP UX on Agenda for this surface. Done (prior model): `[agenda-event-rsvp](specs/archive/agenda-event-rsvp.md)` |
...
| ---- | ------------------------------- | ------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 1    | auto-decline-unofferable        | planned | 2026-08-28 · initial       | Wire auto-decline when own ride is `"requested"` — can't offer a ride you don't have                                        |
| 2    | attendance-manual-toggle        | active  | 2026-08-28 · initial       | Manual not-going toggle only; default going; never a hero item; **going** vocabulary                                         |
...
| 8    | push-notifications              | planned | 2026-08-07 · initial       | Expo push for carpool ride **request / accept / deny (pass)** — pre-beta differentiator; not blocked on in-app inbox          |
...
| 11   | event-arrival-lead-time         | planned | 2026-08-11 · enhancement   | Editable arrival lead times: game ~30m / practice ~15m / other ~0 (defaults); refine leave-by target                          |
...
| Id                            | Cancelled  | Reason |
| ----------------------------- | ---------- | ------ |
| assign-cancels-carpool-request | 2026-08-28 | Superseded by hero & coverage flow redesign — [`auto-decline-unofferable`](specs/planned/auto-decline-unofferable.md) + [`ride-revert-undo`](specs/archive/ride-revert-undo.md) |
...
| Id | Branch | Spec |
| -- | ------ | ---- |
| attendance-manual-toggle | `attendance-manual-toggle` | [active](specs/active/attendance-manual-toggle.md) |
