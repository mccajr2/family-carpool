...
Status: active  
Updated: 2026-08-27 (`/pr agenda-carpool-state-clarity`)
...
| Coverage                 | **Responsibility** rows (adult + kid subset + PENDING/CONFIRMED/DECLINED); any member assigns; assignee confirms/declines; kid exclusive per item on active rows; not a trip/seat plan. API `uncoveredKidIds` stays orthogonal to rides. Full coverage↔ride lifecycle coupling (Assign should not leave a teammate expecting pickup) is planned — `[assign-cancels-carpool-request](specs/planned/assign-cancels-carpool-request.md)`. Done: `[coverage-confirm-decline](specs/archive/coverage-confirm-decline.md)` |
...
| ---- | ------------------------------- | ------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 1    | assign-cancels-carpool-request  | planned | 2026-08-26 · enhancement   | Assign (family coverage) should cancel open carpool ask when those kids are covered — stop teammate false pickup expectation |
