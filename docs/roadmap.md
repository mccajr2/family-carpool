...
Status: active  
Updated: 2026-09-11 (`/pr carpool-leg-split-plans` — archive; chip-collapse shipped early)
...
| Carpool request          | **v1** Done: [`carpool-request-accept`](specs/archive/carpool-request-accept.md) — **one request covering all attending kids who still need a ride** by default (seats = kid count); **override** to drop a kid (e.g. sick). v1 is **both legs** (to and from the event) and **pickup at the requester’s house**. **Recovery** Done: [`garage-retire`](specs/archive/garage-retire.md) (hard-delete garage; Accept open to all adults). **Then Beta (Ride Coverage Card split 2026-09-10):** Done [`carpool-leg-to-from`](specs/archive/carpool-leg-to-from.md) (per-leg domain + four-state chips + not-going clears legs) → Done [`carpool-ride-coverage-card`](specs/archive/carpool-ride-coverage-card.md) (default DriverPicker chrome) → Done [`carpool-leg-split-plans`](specs/archive/carpool-leg-split-plans.md) (split editor + Save ride plan + per-leg hero gaps; matching-chip collapse shipped early via [`carpool-leg-chip-collapse`](specs/archive/carpool-leg-chip-collapse.md)) → per-kid progressive split (`carpool-kid-split-plans`) → meet-at / Ask-team sub-flow (`carpool-meet-at`); then stop-order optimize (`carpool-route-optimize`); recurring (`carpool-recurring-rotation`); neighborhood (`neighborhood-carpool`). Still parking: early/late windows (`carpool-early-late-window`); capacity (`garage-capacity`) |
...
| ---- | ------------------------------- | ------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 1    | carpool-kid-split-plans         | planned | 2026-09-10 · re-rank split | Progressive per-kid plans for multi-kid teams (twins/triplets); default remains shared round-trip                           |
| 2    | carpool-meet-at                 | planned | 2026-08-14 · enhancement   | Ask-team meet-at sub-flow + true per-leg pickup/drop-off places (locations deferred from leg-split)                         |
...
| Date       | Event                                                                                                                                                                                                                             |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-11 | `/pr carpool-leg-split-plans`: split editor + Save ride plan + per-leg hero gaps; circle-local PLAN; waiting-household confirm; matching-chip collapse shipped early. Specs archived (`carpool-leg-split-plans`, `carpool-leg-chip-collapse`). Next up `carpool-kid-split-plans`. |
| 2026-09-11 | `/spec carpool-leg-split-plans`: promote — split editor + per-leg drivers + hero queue gaps; defer true per-leg locations into `carpool-meet-at` (no new id). Branch `carpool-leg-split-plans`. Soft dep: chip-collapse still planned on main. |
