...
| Circle writes            | **Organizer-only:** kids, invite/roles, circle rename, **activity feeds** (+ Sync). **Any member:** **named places**, **manual events**. **Garage vehicle writes:** retired from product until `[garage-retire](specs/planned/garage-retire.md)` then parking `[garage-capacity](specs/planned/garage-capacity.md)` — prior ship: `[garage-vehicles](specs/archive/garage-vehicles.md)`                                                                                                                                                                                      |
...
| Carpool request          | **v1** Done: [`carpool-request-accept`](specs/archive/carpool-request-accept.md) — **one request covering all attending kids who still need a ride** by default (seats = kid count); **override** to drop a kid (e.g. sick). v1 is **both legs** (to and from the event) and **pickup at the requester’s house**. **Recovery Next:** [`garage-retire`](specs/planned/garage-retire.md) (decouple Accept from vehicles/seats; remove Garage nav). **Then Beta:** one-way to XOR from (`carpool-leg-to-from`); pickup vs drop-off at a teammate house (`carpool-meet-at`); multi-stop **stop-order optimize** (`carpool-route-optimize`); recurring rotation (`carpool-recurring-rotation`); neighborhood concept (`neighborhood-carpool`). Still parking: early/late windows (`carpool-early-late-window`); capacity (`garage-capacity`) |
...
| Vehicle specs            | **Parked with capacity.** Prior ship: year/make/model (vPIC), no VIN, overridable total seats — `[garage-vehicles](specs/archive/garage-vehicles.md)`. **Retire from product** via `[garage-retire](specs/planned/garage-retire.md)`. Revive later: parking `[garage-capacity](specs/planned/garage-capacity.md)`; seat kinds `[garage-seat-kinds](specs/planned/garage-seat-kinds.md)`. Do **not** gate Accept on vehicles/seats until capacity returns.                                                                                                                                                                              |
...
| ---- | ------------------------------- | ------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 1    | garage-retire                   | planned | 2026-09-10 · enhancement   | **Recovery:** remove Garage from nav; decouple Accept from vehicles/seats/`vehicleId`; retire garage API/UI (not dormant keep) |
