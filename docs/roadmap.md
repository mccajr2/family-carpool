...
Status: active  
Updated: 2026-09-16 (`/spec family-conflict-report`)
...
1. **Calendar** — unified schedule across kids/activities; overridable
  leave-from places; estimated leave-by; conflict detection; coverage
   confirmation.
2. **Carpool** — opt-in team spaces (parent invite); simple **request /
  accept** (coverage, teammate rides). **Vehicles / seats / garage** are
  future capacity (`garage-capacity`) — not part of the current loop.
...
| Roles                    | **Organizer** (invite/remove, manage feeds/kids) + **Caregiver** (calendar, coverage, carpool). Driver-only later. Garage/capacity retired from current Caregiver chrome — revive with `[garage-capacity](specs/planned/garage-capacity.md)`                                                                                                                                                                                                                                                                                                                                    |
| Circle writes            | **Organizer-only:** kids, invite/roles, circle rename, **activity feeds** (+ Sync). **Any member:** **named places**, **manual events**. **Garage vehicle writes:** retired Done `[garage-retire](specs/archive/garage-retire.md)`; revive later via parking `[garage-capacity](specs/planned/garage-capacity.md)` — prior ship: `[garage-vehicles](specs/archive/garage-vehicles.md)`                                                                                                                                                                                      |
| Driving                  | **Recovery lock (2026-09-10):** Accept/Decline stay visible to **all** circle adults — no “I drive / don’t drive” gate, no vehicle/`vehicleId` requirement for dogfood. Prior garage model (0+ vehicles, owner + explicit drivers) archived in `[garage-vehicles](specs/archive/garage-vehicles.md)`; revive only via parking `[garage-capacity](specs/planned/garage-capacity.md)`                                                                                                                                                                                          |
| Same team, multiple kids | Attach **which kids** belong on a feed/team; calendar shows both                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Carpool request          | **v1** Done: [`carpool-request-accept`](specs/archive/carpool-request-accept.md) — **one request covering all attending kids who still need a ride** by default (seats = kid count); **override** to drop a kid (e.g. sick). v1 is **both legs** (to and from the event) and **pickup at the requester’s house**. **Recovery** Done: [`garage-retire`](specs/archive/garage-retire.md) (hard-delete garage; Accept open to all adults). **Then Beta (Ride Coverage Card split 2026-09-10):** Done [`carpool-leg-to-from`](specs/archive/carpool-leg-to-from.md) (per-leg domain + four-state chips + not-going clears legs) → Done [`carpool-ride-coverage-card`](specs/archive/carpool-ride-coverage-card.md) (default DriverPicker chrome) → Done [`carpool-leg-chip-collapse`](specs/archive/carpool-leg-chip-collapse.md) (plain status when both ways match) → Done [`carpool-leg-split-plans`](specs/archive/carpool-leg-split-plans.md) (split editor + Save ride plan + per-leg hero gaps) → Done [`carpool-kid-split-plans`](specs/archive/carpool-kid-split-plans.md) (progressive per-kid plans) → Done [`carpool-leg-places`](specs/archive/carpool-leg-places.md) (per-leg family-side places) → Done [`carpool-meet-at`](specs/archive/carpool-meet-at.md) (Ask-team meet side; bind on Accept) → Done [`carpool-route-optimize`](specs/archive/carpool-route-optimize.md) (stop-order optimize + reusable waypoint helper) → Done [`day-block-domain`](specs/archive/day-block-domain.md) (drive-time-aware merge + FORCE_MERGE/SPLIT; interim Agenda link) → Done [`day-block-agenda`](specs/archive/day-block-agenda.md) (one Agenda card per block; Focus stays one-decision) → then `day-block-route` ([ADR-0004](decisions/ADR-0004-carpool-card-perspective-rules.md)); then recurring (`carpool-recurring-rotation`); neighborhood (`neighborhood-carpool`). Still parking: early/late windows (`carpool-early-late-window`); capacity (`garage-capacity`) |
...
| Client ship order        | **Web is the product reference**; new verticals land on web first. **Carpool Beta on web first** — Expo scaffold + push are **parked** until the carpool product cluster dogfoods (not a gate that reorders product ahead of leave-from / one-way / recurring). When revived: [`rn-expo-scaffold`](specs/planned/rn-expo-scaffold.md) then [`push-notifications`](specs/planned/push-notifications.md). **Do not** lockstep every feature on web + RN. **Agenda block chrome:** Done web client-only over event-shaped `driveBlock*` ([`day-block-agenda`](specs/archive/day-block-agenda.md)); server block-shaped contract is parking [`agenda-block-api`](specs/planned/agenda-block-api.md) — **hard dependency** before any RN Agenda consumes grouped blocks (gated on merge-rule dogfood). KMP Android/iOS are **frozen** — cancelled KMP `*-mobile` ports; no new Compose/SwiftUI or `sharedLogic` OpenAPI work. Remove KMP via [`kmp-mobile-retire`](specs/planned/kmp-mobile-retire.md). Contract same-change rule: **web** (+ Expo when it exists), not KMP — see `AGENTS.md`. |
...
| Vehicle specs            | **Parked with capacity.** Prior ship: year/make/model (vPIC), no VIN, overridable total seats — `[garage-vehicles](specs/archive/garage-vehicles.md)`. **Retired from product** Done `[garage-retire](specs/archive/garage-retire.md)`. Revive later: parking `[garage-capacity](specs/planned/garage-capacity.md)`; seat kinds `[garage-seat-kinds](specs/planned/garage-seat-kinds.md)`. Do **not** gate Accept on vehicles/seats until capacity returns.                                                                                                                                                                              |
...
| Interaction UX           | Distinct custom UI guided by UX-law tenets (not a rideshare clone): **Aesthetic-Usability** (polish reads as usable), **Doherty** (focused busy feedback feels instant), **Fitts** (primary actions large/reachable), **Hick** (few choices per step; sole-option defaults), **Proximity / Similarity** (group related controls; consistent patterns). Calendar/Agenda is the living reference: `[calendar-ux-flow](specs/archive/calendar-ux-flow.md)`. **Calendar-primary rides:** Accept/Request on Focus/Agenda is not enough — **reverse actions** (Cancel own request, Withdraw when we accepted) must stay reachable on Calendar when that ride state is shown (Focus is not duplicated in the day list). Follow-ups: Done `[agenda-carpool-action-parity](specs/archive/agenda-carpool-action-parity.md)`, `[agenda-carpool-state-clarity](specs/archive/agenda-carpool-state-clarity.md)`; cancelled `[assign-cancels-carpool-request](specs/planned/assign-cancels-carpool-request.md)` → Done `[auto-decline-unofferable](specs/archive/auto-decline-unofferable.md)`. Inspired by [Laws of UX that Uber follows](https://medium.com/design-bootcamp/laws-of-ux-that-uber-follows-fa7c6619748b) |
| Visual language          | Destination mocks are the visual source of truth for **size, weight, spacing, and color** — tokens absorb those values (new/updated roles in the same PR); do not snap to a nearby existing role. **WCAG AA** (4.5:1 text, 3.0:1 icons) is the only allowed mock-hex exception. See `[docs/ui-system.md](ui-system.md)`. **One visual priority per screen**; everything else calmer. Destructive actions (Remove/Delete) get **less** weight than neutral (Edit/Sync). A design pass restyles only — **same handlers**. Web typography: Space Grotesk (display) + Plus Jakarta Sans (body) — `[typography-web](specs/archive/typography-web.md)`. Expo typography/fonts are deferred until RN surfaces exist (KMP `typography-font-family` cancelled). `hero*` **color** roles are Focus-card urgent spotlight only. **Status chips (product-wide):** governing language is **Feeds** — **uppercase**, **no leading bullet/dot**, same size/weight/padding as feed carpool chips (`feedChip*` tokens / `CarpoolFeedActions`). Agenda Focus + collapsed rows must match; Title Case + leading-dot **pills** from `[agenda-list-chips](specs/archive/agenda-list-chips.md)` / Focus polish are **superseded** — Done: `[agenda-chip-section-headers](specs/archive/agenda-chip-section-headers.md)`. **Filters and states prefer chips** over body-copy labels on restyled surfaces. |
| Web shell rail           | Signed-in web chrome is an **always-dark docked left rail** (independent of page theme): Calendar / Carpool / Family with icons; Settings **Places / Feeds** (**no Garage** — Done `[garage-retire](specs/archive/garage-retire.md)`); **ACCOUNT** footer (avatar, email, role, sign out) **always visible** — pinned; nav list may scroll, Sign out must not. Expo will use bottom tabs (IA parity, not KMP). Rail wordmark is placeholder chrome (accent mark) — do not lock copy; real name is `[app-identity-rename](specs/planned/app-identity-rename.md)`. Done: `[web-shell-nav-rail](specs/archive/web-shell-nav-rail.md)` (Garage row was in that ship; retire removes it). **Page frame:** flush-left `md:w-60` rail, uncarded main (`max-w-[820px]`), Calendar-only Context aside — `[web-shell-page-frame](specs/archive/web-shell-page-frame.md)`. **Week at a glance:** seven-day coverage/status strip in that aside — `[agenda-week-glance](specs/archive/agenda-week-glance.md)`. **Page header:** Calendar mock Today/date, `page`/`subtitle` type, main 36×44 / rail 28×20 padding — `[web-shell-page-header](specs/archive/web-shell-page-header.md)`. |
| Agenda week strip        | **Five-day coverage/status** rail in Calendar Context (today + next four local days; event counts, not kids; no driver copy). Not the month/week grid (`family-calendar-grid`). Done: `[agenda-week-glance](specs/archive/agenda-week-glance.md)`. KMP mobile port cancelled; Expo port carved later if needed. Confirmed-ride **Route** detail (ordered stops, maps deep link, leave-by with pickups): Done `[ride-detail-shell](specs/archive/ride-detail-shell.md)` → Done `[ride-route-tab](specs/archive/ride-route-tab.md)` (supersedes cancelled `carpool-multi-stop`) → Done `[ride-playlist-tab](specs/archive/ride-playlist-tab.md)` (Spotify shipped, then **parked** — see Carpool music) → Done `[ride-detail-route-only](specs/archive/ride-detail-route-only.md)` (Playlist chrome hidden). Route polish: `[ride-detail-polish](specs/planned/ride-detail-polish.md)`. Mockup SoT: `[docs/ui-system/carpool-combined-flow.jsx](ui-system/carpool-combined-flow.jsx)`. |
...
| ---- | ------------------------------- | ------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 1    | family-conflict-report          | active  | 2026-09-16 · enhancement   | Different kids’ overlapping events → clear lower-urgency Agenda report (who + events); no forced Hero; not adult-coverage amber |
| 2    | day-block-route                 | planned | 2026-09-14 · enhancement   | Block Route spans full multi-event stop sequence; extend optimize to block stops ([ADR-0004](decisions/ADR-0004-carpool-card-perspective-rules.md)) |
| 3    | manual-event-team-link          | planned | 2026-08-13 · re-rank split | Manual events: attach to a team (feed UUID, carpool-eligible) or standalone — prerequisite for recurring rotation             |
| 4    | carpool-recurring-rotation      | planned | 2026-08-16 · enhancement   | Standing teammate rotation for a recurring team event; RSVP No drops that kid for that week only                              |
| 5    | neighborhood-carpool            | planned | 2026-09-08 · enhancement   | Neighborhood carpool concept (Claude mockup SoT — attach under `docs/ui-system/`); may split at `/spec`                        |
| 6    | ride-detail-polish              | planned | 2026-09-06 · enhancement   | Route loading / notify errors / explicit OSRM-unreachable leave-by (playlist chrome parked)                                   |
| 7    | event-arrival-lead-time         | planned | 2026-08-11 · enhancement   | Editable arrival lead times; reconcile Agenda + Route `bufferMinutes`                                                         |
| 8    | conflict-travel-margin          | planned | 2026-08-12 · enhancement   | Soft "cutting it close" warn from leave-by/travel gaps (after leave-from / lead-time)                                         |
| 9    | auth-email-delivery             | planned | 2026-08-07 · enhancement   | Production SMTP/API mail for OTP — still needed for real-user Beta; not blocking carpool product dogfood                      |
| 10   | client-server-invariant-audit   | planned | 2026-08-31 · enhancement   | Audit “must not happen” rules — UI-only vs API-enforced; punch-list server fences before a second client                      |
| 11   | web-auth-session-hardening      | planned | 2026-08-07 · enhancement   | HTTP-only cookie (or equivalent) for web — pre-beta gate; Expo stays Bearer when revived                                      |
...
| apple-music-connect           | 2026-09-08 · re-rank split | Secure Apple Music / MusicKit developer credentials + catalog query + test-user authorize (dogfood with existing Apple Music sub; ~$99/yr Apple Developer Program) |
| apple-music-playlist-ingest   | 2026-09-08 · re-rank split | Designate/read Apple Music playlists → normalized tracks; re-enable Playlist participation UI on ride detail (reuse rider tiles / one playlist per kid)            |
| apple-music-web-playback      | 2026-09-08 · re-rank split | Duration-bounded (~60–90 min) ride mix via existing merge; web playback (prefer ephemeral queue over permanent provider playlist)                                 |
| apple-music-native-playback   | 2026-09-08 · re-rank split | Expo/iOS/Android MusicKit playback + background while Maps is foregrounded — validate early (after `rn-expo-scaffold`); not assumed solved by web API             |
| music-playback-host           | 2026-09-08 · re-rank split | Participation vs playback: many contribute preferences/playlists; one supported host plays the mix — exact no-provider / host-leaves behavior deferred to `/spec` |
| member-profile-photos         | 2026-08-26 · enhancement | Real parent/teammate/kid images for card circles — after names/initials dogfood (Done: `agenda-ride-rider-chips`); needs upload/storage product work              |
...
| ui-system-destination-adoption | 2026-08-10 · note carried from cross-platform-ui-system approval | Remainder token adoption (Carpool / Family / Places / grid) — Garage out until `garage-capacity`; parked with other design work; Expo adopts separately when surfaces exist |
...
| in-app-notifications        | 2026-08-16 · enhancement | In-app alert inbox (coverage, rides, rotation gaps); **not** required before carpool push — promote after push if inbox UX is needed                               |
...
| carpool-page-redesign       | 2026-08-16 · enhancement | Carpool destination visual restyle — needs mockups / design intent before `/spec` (sibling of Family/Places)                  |
...
| driver-only-role            | 2026-08-07 · initial     | Narrow Driver-only role — defer until after `garage-capacity` (no drive flag while Accept is open to all adults)              |
...
| osm-map-tiles               | 2026-08-07 · initial     | Optional in-app map with OSM tiles + attribution (Route tab uses Embed API or placeholder; not this)                                                              |
...
| locker-room-mix             | 2026-08-14 · enhancement | Team pump-up mix using the same playlist engine as ride merge — after Apple Music / provider path dogfoods                                                        |
| garage-capacity             | 2026-09-10 · enhancement | **Future:** vehicles + seating capacity after carpool dogfood — do **not** gate Accept until promoted; follows `garage-retire` |
| garage-seat-kinds           | 2026-08-14 · enhancement | Distinguish adult seats, kid seats, and boosters — only after `garage-capacity`                                               |
| family-places-garage-redesign | 2026-08-15 · re-rank split | Family / Places visual restyle (Garage out until capacity returns) — needs mockup pass before `/spec`                       |


## Cancelled

Do not promote. Includes KMP ports superseded by Expo, plus product ids absorbed
by the ride Route/Playlist carve-up.


| Id                            | Cancelled  | Reason |
| ----------------------------- | ---------- | ------ |
| playlist-open-in-streaming    | 2026-09-08 | Superseded by parked Apple Music / provider path (`music-provider-model` → `apple-music-*`); free multi-service lookup no longer the next music slice |
| carpool-multi-stop            | 2026-09-06 | Superseded by approved Route/Playlist flow — [`ride-route-tab`](specs/archive/ride-route-tab.md) |
| driver-leave-by-pickups       | 2026-09-06 | Absorbed into [`ride-route-tab`](specs/archive/ride-route-tab.md) (multi-stop leave-by) |
| maps-deep-links               | 2026-09-06 | Absorbed into [`ride-route-tab`](specs/archive/ride-route-tab.md) (Google Maps universal deep link) |
| ride-playlist-merge           | 2026-09-06 | Superseded by [`ride-playlist-tab`](specs/archive/ride-playlist-tab.md) (mockup round-robin merge) |
| assign-cancels-carpool-request | 2026-08-28 | Superseded by hero & coverage flow redesign — [`auto-decline-unofferable`](specs/archive/auto-decline-unofferable.md) + [`ride-revert-undo`](specs/archive/ride-revert-undo.md) |
...
| Id                     | Branch                   | Spec                                                   |
| ---------------------- | ------------------------ | ------------------------------------------------------ |
| family-conflict-report | `family-conflict-report` | [active](specs/active/family-conflict-report.md)       |
...
| day-block-agenda           | 2026-09-16 | [archive](specs/archive/day-block-agenda.md)           |
| day-block-domain           | 2026-09-15 | [archive](specs/archive/day-block-domain.md)           |
| carpool-route-optimize     | 2026-09-15 | [archive](specs/archive/carpool-route-optimize.md)     |
...
| carpool-leg-split-plans    | 2026-09-11 | [archive](specs/archive/carpool-leg-split-plans.md)    |
| carpool-leg-chip-collapse  | 2026-09-11 | [archive](specs/archive/carpool-leg-chip-collapse.md)  |
| carpool-ride-coverage-card | 2026-09-11 | [archive](specs/archive/carpool-ride-coverage-card.md) |
| carpool-leg-to-from        | 2026-09-10 | [archive](specs/archive/carpool-leg-to-from.md)        |
| garage-retire              | 2026-09-10 | [archive](specs/archive/garage-retire.md)              |
| coverage-priority-same-event | 2026-09-09 | [archive](specs/archive/coverage-priority-same-event.md) |
...
| ride-detail-shell          | 2026-09-07 | [archive](specs/archive/ride-detail-shell.md)          |
...
| weekly-list-focus-sync     | 2026-08-30 | [archive](specs/archive/weekly-list-focus-sync.md)     |
| hero-attention-carousel    | 2026-08-28 | [archive](specs/archive/hero-attention-carousel.md)    |
| unified-ride-status-chip   | 2026-08-28 | [archive](specs/archive/unified-ride-status-chip.md)   |
...
| agenda-carpool-action-parity | 2026-08-26 | [archive](specs/archive/agenda-carpool-action-parity.md) |
| carpool-pass-reconsider    | 2026-08-26 | [archive](specs/archive/carpool-pass-reconsider.md)     |
...
| 2026-09-16 | `/pr player-conflict-hero`: Hero keep A/B + shared neither for same-kid overlap; multi-kid progressive; ADR-0001 + contract queue precedence. Spec archived. Next up `family-conflict-report`. |
| 2026-09-16 | `/spec player-conflict-hero`: re-promote after `hero-not-going` Done — client-only Hero from `KID_TIME_OVERLAP`; keep A/B + shared neither; multi-kid progressive; amends ADR-0001. Branch `player-conflict-hero`. |
| 2026-09-16 | `/pr hero-not-going`: own-kid Hero gap + Confirm secondary not-going (1 kid / 2+ all-kids + Different plans); reuse `setCalendarRsvp`; Confirm progressive is per-kid not-going only. Spec archived. Next up `player-conflict-hero`. |
| 2026-09-16 | `/spec hero-not-going`: promote — Hero gap + Confirm not-going via secondary link (1 kid) / Different plans progressive (2+); reuse `setCalendarRsvp`; prereq for player-conflict. Branch `hero-not-going`. |
| 2026-09-16 | `/spec player-conflict-hero` **re-rank split**: carve `hero-not-going` (rank 1 — own-kid Hero not-going escape); demote fleshed `player-conflict-hero` to planned rank 2 (depends on it). Next up `hero-not-going`. |
| 2026-09-16 | `/spec player-conflict-hero`: flesh — client-only Hero from `KID_TIME_OVERLAP`; force keep / not-going; multi-kid progressive default-same + split; horizon same as other Hero; then normal ride-assign slides. Then split for Hero not-going consistency. Branch `player-conflict-hero`. |
| 2026-09-16 | `/pr day-block-agenda`: Agenda one card per driving block + muted “not your job” band; Focus stays one-decision; merge/split on block card only. Spec archived. Next up was `day-block-route` (main now ranks schedule conflicts ahead). |
| 2026-09-16 | `/roadmap` schedule conflicts: dogfood (two teams) — add ranks 1–2 `player-conflict-hero` (Hero force pick + ride assign on same-kid overlap) → `family-conflict-report` (multi-kid overlap, report-only); lock Schedule conflicts; bump day-block chain. Next up `player-conflict-hero`. |
| 2026-09-16 | `/spec day-block-agenda`: promote — web-only one card per block over `driveBlock*` (no OpenAPI); ADR-0004 + mockup SoT; remove interim Agenda links; park follow-up `agenda-block-api` (gated on merge-rule dogfood; hard dep before RN Agenda blocks). Branch `day-block-agenda`. |
| 2026-09-15 | `/pr day-block-domain`: driving-block compute + FORCE_MERGE/SPLIT overrides + interim Agenda link; OpenAPI/web client. Spec archived. Next up `day-block-agenda`. |
...
| 2026-09-15 | `/pr carpool-route-optimize`: reusable stop-sequence optimizer + single-event Route auto-order; driver drag until fingerprint rebuild; day-block stub notes reuse. Spec archived. Next up `day-block-domain`. |
...
| 2026-09-14 | `/spec carpool-meet-at` re-rank split: rank 1 **active** [`carpool-leg-places`](specs/archive/carpool-leg-places.md) (true per-leg places); rank 2 narrowed `carpool-meet-at` (Ask-team meet-at only); bump route-optimize + day-block. Branch `carpool-leg-places`. |
| 2026-09-14 | `/roadmap` day-block grouping intake: ranks 3–5 `day-block-domain` → `day-block-agenda` → `day-block-route` (after unchanged meet-at / route-optimize); ADR-0004 perspective/leg/grouping rules; mockups under `docs/ui-system/`. Next up still `carpool-meet-at`. |
| 2026-09-13 | `/pr carpool-kid-split-plans`: progressive per-kid DriverPicker; multiple RideRequests grouped by identical TO/FROM; per-kid hero gaps/chips; `ownRequests` contract. Spec archived. Next up `carpool-meet-at`. |
...
| 2026-09-11 | `/pr carpool-leg-chip-collapse`: matching TO/FROM → one plain status body (no Round trip prefix); dual prefixed chips when legs differ or single-leg. Spec archived. Next up `carpool-leg-split-plans`. |
...
| 2026-09-10 | `/pr garage-retire`: hard-delete Garage/vPIC (API/UI/schema); Accept empty-body for all space-member adults; anti-leak fences until `garage-capacity`. Spec archived. Next up `carpool-leg-to-from`. |
| 2026-09-10 | Major `/roadmap` recovery: garage was premature and still gates Accept — add rank-1 `garage-retire` (remove nav + API/UI; Accept open to all adults, no drive toggle); park `garage-capacity` + keep `garage-seat-kinds` parked; bump `carpool-leg-to-from` to rank 2; lock Vision/Driving/Vehicle/Web shell. Next up `garage-retire`. |
| 2026-09-09 | `/pr coverage-priority-same-event`: event-grouped `getQueue` (own gaps then asks per event); ADR-0001 amended. Spec archived. Next up `carpool-leg-to-from`. |
| 2026-09-09 | `/spec coverage-priority-same-event`: active — event-grouped `getQueue` (own gaps then asks per event, soonest first); amends ADR-0001. Branch `coverage-priority-same-event`. |
...
| 2026-09-08 | `/roadmap` park Playlist: Spotify Premium/dev constraints — hide Playlist UI (`ride-detail-route-only` Next up); narrow `ride-detail-polish` to Route; lock Carpool music + split provider-agnostic Apple Music path into parking (`music-provider-model` → `apple-music-*` → `music-playback-host`); cancel `playlist-open-in-streaming`. Pre-beta mail/Expo unchanged after Route polish. |
...
| 2026-08-26 | `/pr carpool-pass-reconsider`: Pass = soft decline (Focus skip); Accept after Pass on Carpool tab; requester “Passed by …”. Spec archived. Next up `agenda-carpool-action-parity`. |
| 2026-08-26 | `/roadmap assign-cancels-carpool-request`: dogfood follow-up — Assign coverage leaving an open carpool ask is unsafe (teammate may still expect pickup). Rank 4; open product questions before `/spec`. |
| 2026-08-26 | `/roadmap` + `/spec agenda-chip-section-headers`: promote Feeds-style chip + section-header chrome to rank 1 (ahead of carpool density / action-parity PR). Spec active. Next up `agenda-chip-section-headers`. |
| 2026-08-26 | `/spec agenda-carpool-action-parity`: Calendar reverse-action self-sufficiency — Focus Cancel (own PENDING/ACCEPTED) + Withdraw (accepted-by-us); row Withdraw when that state shown; Carpool Pass CTA. No Accept/Pass on Agenda rows; no OpenAPI. |
| 2026-08-26 | `/pr agenda-carpool-action-parity`: Focus Cancel/Withdraw + AgendaRow Withdraw + Carpool Pass CTA. Spec archived. Next up `agenda-carpool-state-clarity`. |
| 2026-08-26 | `/pr agenda-chip-section-headers`: Agenda Feeds-style uppercase chips (no pills) + section labels NEEDS YOUR ATTENTION…LATER; design docs locked. Spec archived. Next up `agenda-carpool-state-clarity`. |
...
| 2026-08-28 | `/pr unified-ride-status-chip`: unified ride-status chip helpers for Focus + collapsed AgendaRow; **You're driving · +N** route tone; separate carpool-ask chip. Spec archived. Next up `hero-attention-carousel`. |
| 2026-08-28 | `/pr hero-attention-carousel`: swipeable "Needs your attention" carousel over `getQueue`; every slide independently actionable; empty → all caught up. Spec archived. Next up `weekly-list-focus-sync`. |
| 2026-08-30 | `/pr weekly-list-focus-sync`: GameCard list rows; queue items in list + carousel; focus ring syncs to `getQueue[0]`; expanded gap → DriverPicker; inbound request band. Spec archived. Next up `ride-revert-undo`. |
| 2026-08-30 | `/spec ride-revert-undo`: RevertRideLink + inbound Reconsider/Undo; ADR-0002 atomic remove-coverage auto-withdraw. Spec active. Next up `/implement ride-revert-undo`. |
| 2026-08-31 | `/pr ride-revert-undo`: RevertRideLink + inbound Can't take them / Reconsider / Undo; ADR-0002 remove-coverage auto-withdraws accepted inbound. Spec archived. Next up `auto-decline-unofferable`. |
| 2026-08-31 | `/spec auto-decline-unofferable`: wire `autoDeclineUnofferable` + Assign cancels open team ask; session `autoDeclined` for Reconsider. Spec active. Next up `/implement auto-decline-unofferable`. |
| 2026-08-31 | `/roadmap`: add pre-beta [`client-server-invariant-audit`](specs/planned/client-server-invariant-audit.md) at rank 7 (after auth mail, before Expo). Active `auto-decline-unofferable` unchanged. |
| 2026-08-31 | `/pr auto-decline-unofferable`: session `autoDeclined` + Assign→cancel open team ask; wire `autoDeclineUnofferable` on remap. Spec archived. Next up `attendance-manual-toggle`. |
...
| 2026-09-01 | `/pr coverage-copy-a11y-polish`: shared `coverageCopy`, DriverPicker separation, hero carousel a11y, light+dark contrast gate (`heroOnInverse`, agenda chip pairings). Spec archived. Next up `agenda-ride-rider-chips`. |
| 2026-09-01 | `/pr agenda-ride-rider-chips`: Agenda RiderChips (initials + first names) on collapsed/expanded rows; drop `· +N` suffix when riders visible. Spec archived. Next up `ride-commitment-conflict`. |
...
| 2026-09-06 | `/spec ride-detail-shell`: promote stub — Agenda `canRoute` entry + Detail Route/Playlist chrome with full mockup UI on fixtures (no OSRM/Spotify/OpenAPI). Spec active on branch `ride-detail-shell`. Next up `/implement ride-detail-shell`. |
| 2026-09-07 | `/pr ride-detail-shell`: Agenda `canRoute` entry + Detail Route/Playlist overlay on fixtures; tokens locked; no OpenAPI. Spec archived. Next up `ride-route-tab`. |
| 2026-09-07 | `/spec ride-route-tab`: promote stub — live multi-stop Route (OpenAPI + leaveby OSRM, cache-on-confirm, buffers 45/20/0, notify call-site only). Spec active on branch `ride-route-tab`. Next up `/implement ride-route-tab`. |
| 2026-09-07 | `/pr ride-route-tab`: calendar route GET + leaveby multi-stop OSRM cache-on-confirm; web Route tab live stops/leave-by + UNAVAILABLE; notify local-only. Spec archived. Next up `ride-playlist-tab`. |
| 2026-09-07 | `/spec ride-playlist-tab`: promote stub — live Playlist (Spotify OAuth, per-kid designated playlist, 1→open / 2+→transient merge, Remix for 2+, invite soft-fail). Spec active on branch `ride-playlist-tab`. Next up `/implement ride-playlist-tab`. |
| 2026-09-07 | `/pr ride-playlist-tab`: Spotify OAuth + per-kid designate + live Playlist read/open/remix; invite soft-fail. Spec archived. Next up `ride-detail-polish`. |
