...
Status: active  
Updated: 2026-09-16 (`/spec` day-block-agenda)
...
| Client ship order        | **Web is the product reference**; new verticals land on web first. **Carpool Beta on web first** — Expo scaffold + push are **parked** until the carpool product cluster dogfoods (not a gate that reorders product ahead of leave-from / one-way / recurring). When revived: [`rn-expo-scaffold`](specs/planned/rn-expo-scaffold.md) then [`push-notifications`](specs/planned/push-notifications.md). **Do not** lockstep every feature on web + RN. **Agenda block chrome:** web ships client-only over event-shaped `driveBlock*` ([`day-block-agenda`](specs/active/day-block-agenda.md)); server block-shaped contract is parking [`agenda-block-api`](specs/planned/agenda-block-api.md) — **hard dependency** before any RN Agenda consumes grouped blocks (gated on merge-rule dogfood). KMP Android/iOS are **frozen** — cancelled KMP `*-mobile` ports; no new Compose/SwiftUI or `sharedLogic` OpenAPI work. Remove KMP via [`kmp-mobile-retire`](specs/planned/kmp-mobile-retire.md). Contract same-change rule: **web** (+ Expo when it exists), not KMP — see `AGENTS.md`. |
...
| ---- | ------------------------------- | ------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 1    | day-block-agenda                | active  | 2026-09-14 · enhancement   | Agenda/Focus: one card per block; per-leg breakdown + quiet "not your job" rows; web-only over `driveBlock*` (no OpenAPI); mockup SoT `docs/ui-system/day-block-grouping.mockup.html` |
...
| Id | Branch | Spec |
| -- | ------ | ---- |
| day-block-agenda | `day-block-agenda` | [active](specs/active/day-block-agenda.md) |
