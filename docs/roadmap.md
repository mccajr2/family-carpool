...
Status: active  
Updated: 2026-08-21 (`/spec agenda-focus-carpool-actions`)
...
**Clients (beta):** **Web-first for new product verticals** (OpenAPI →
backend → web). Solidify the loop on web, then port to Android/iOS. Visual
restyles and mobile ports of existing web design stay parked until carpool
is dogfoodable on web.
...
| Focus card selection     | Exactly one Agenda item. **Next action**, not “earliest uncovered in the loaded window.” **Today/tomorrow** decisions (RSVP / uncovered / conflict / pending confirm / **pending ride accept for self**) beat a sooner all-set item; otherwise the **next in-play event to leave for** (on time first). Rest-of-week gaps surface in the list + `[agenda-week-glance](specs/archive/agenda-week-glance.md)` strip, not Focus. Done: `[agenda-focus-next-action](specs/archive/agenda-focus-next-action.md)`. **Agenda-primary carpool** (active `[agenda-focus-carpool-actions](specs/active/agenda-focus-carpool-actions.md)`): Request/status on event cards; Focus Accept + Pass; **Within Today/Tomorrow** family decisions beat teammate ride Accept, then earliest `startsAt`; **own PENDING ride is not a Focus decision**. Never multi-hero or Context ask inbox. Focus chrome: slim summary + one CTA (Assign/Confirm/Accept); write bands stay on expanded rows — `[agenda-focus-card-polish](specs/archive/agenda-focus-card-polish.md)`. |
...
| ---- | ------------------------------- | ------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 1    | agenda-focus-carpool-actions    | active  | 2026-08-17 · enhancement   | Agenda-primary carpool (web): Request/status on event cards; teammate asks via Focus Accept + per-adult Pass — not Carpool-tab-only |
...
| Id                            | Added                    | Summary                                                                                                                                                           |
| ----------------------------- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| carpool-request-accept-mobile | 2026-08-18 · enhancement | iOS + Android port of request/accept after the web loop is dogfoodable                                                                                            |
| agenda-focus-card-mobile      | 2026-08-14 · enhancement | iOS + Android port of the polished Focus card — parked until web carpool is dogfoodable                                                                           |
| agenda-full-page-redesign-mobile | 2026-08-15 · enhancement | iOS + Android day-grouped rows including list chips — parked until web carpool is dogfoodable                                                                  |
| feeds-page-redesign-mobile    | 2026-08-15 · enhancement | iOS + Android port of Feeds card restyle — parked until web carpool is dogfoodable                                                                                |
| agenda-week-glance-mobile     | 2026-08-17 · enhancement | iOS + Android five-day status strip — parked until web carpool is dogfoodable                                                                                     |
| family-calendar-grid          | 2026-08-10 · re-rank split | iOS-style month/week calendar grid — parked (design surface; Agenda on web is enough for carpool dogfood)                                                       |
| ui-system-destination-adoption | 2026-08-10 · note carried from cross-platform-ui-system approval | Remainder token adoption (Carpool / Family / Places / Garage / grid) — parked with other design work                                            |
| typography-font-family        | 2026-08-14 · enhancement | iOS + Android font pairing (asset bundling); web shipped as `typography-web` — parked with mobile design                                                          |
...
| Id | Branch | Spec |
| --- | --- | --- |
| agenda-focus-carpool-actions | `agenda-focus-carpool-actions` | [active](specs/active/agenda-focus-carpool-actions.md) |
