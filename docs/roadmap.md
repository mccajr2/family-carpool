...
| Carpool request          | **v1** Done: [`carpool-request-accept`](specs/archive/carpool-request-accept.md) — **one request covering all attending kids who still need a ride** by default (seats = kid count); **override** to drop a kid (e.g. sick). v1 is **both legs** (to and from the event) and **pickup at the requester’s house**. **Beta Upcoming:** one-way to XOR from (`carpool-leg-to-from`); pickup vs drop-off at a teammate house (`carpool-meet-at`); multi-stop **stop-order optimize** (`carpool-route-optimize`); recurring rotation (`carpool-recurring-rotation`); neighborhood concept (`neighborhood-carpool`). Still parking: early/late windows (`carpool-early-late-window`) |
...
| Leave-by                 | Routed duration (OSRM) or fallback + **time-of-day multiplier** + **fixed buffer**; UI labeled **estimate** (not live traffic). Destination coords: **geocode event** `location` (soft-fail); origins = named places. Origin order: per-item override → **per-adult default leave-from** → first located by name. **Agenda must not wait on leave-by** — schedule first, estimates async, near-term before later days. Done: `[agenda-leave-by-async](specs/archive/agenda-leave-by-async.md)`. **Carpool Beta (after Route-only finish):** leave-from UX — **default place vs other named place vs one-time** (and per-coverage when adults take separate cars/kids) — `[coverage-leave-from](specs/planned/coverage-leave-from.md)`                                                                                                 |
...
| ---- | ------------------------------- | ------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 1    | ride-detail-route-only          | planned | 2026-09-08 · enhancement   | Finish in-flight: hide Playlist chrome; Route-only detail (then carpool Beta cluster)                                         |
| 2    | coverage-leave-from             | planned | 2026-08-12 · enhancement   | Leave-from: **default place vs other named place vs one-time**; per-coverage when adults take separate cars/kids               |
| 3    | coverage-priority-same-event    | planned | 2026-09-08 · enhancement   | After own coverage for event E, pending carpool asks for **E** before own gaps on later events (amends ADR-0001)              |
| 4    | carpool-leg-to-from             | planned | 2026-08-14 · enhancement   | One-way carpools: request **to**, **from**, or **both** legs (v1 is always both)                                              |
| 5    | carpool-meet-at                 | planned | 2026-08-14 · enhancement   | Pickup at requester’s house vs drop-off at a teammate’s house (who drives to whom)                                            |
...
| Date       | Event                                                                                                                                                                                                                             |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-08 | Major `/roadmap` carpool Beta re-rank: after in-flight `ride-detail-route-only`, leave-from → same-event priority → one-way → meet-at → stop-order optimize → team-link → recurring → neighborhood; park Expo/push + calendar ETag/venue polish; playlist stays parked. Next product slice `coverage-leave-from`. |
