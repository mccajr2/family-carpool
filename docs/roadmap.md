...
Status: active  
Updated: 2026-08-28 (`/roadmap` hero & coverage flow redesign import)
...
| Coverage                 | **Responsibility** rows (adult + kid subset + PENDING/CONFIRMED/DECLINED); any member assigns; assignee confirms/declines; kid exclusive per item on active rows; not a trip/seat plan. API `uncoveredKidIds` stays orthogonal to rides until later ranks wire lifecycle coupling. **Hero & coverage redesign (2026-08-28):** shared priority queue [`coverage-priority-engine`](specs/active/coverage-priority-engine.md) + [`auto-decline-unofferable`](specs/planned/auto-decline-unofferable.md) + [`ride-revert-undo`](specs/planned/ride-revert-undo.md) supersede `[assign-cancels-carpool-request](specs/planned/assign-cancels-carpool-request.md)` for ride/coverage coupling. Done: `[coverage-confirm-decline](specs/archive/coverage-confirm-decline.md)` |
| Event RSVP               | **Hero & coverage redesign (2026-08-28):** per kid + event attendance defaults **going**; **not going** is manual-only, never a hero item, no "not sure" state — [ADR-0003](decisions/ADR-0003-attendance-manual-default-going.md), [`attendance-manual-toggle`](specs/planned/attendance-manual-toggle.md). Assigning a real driver resets going. **Until rank 8 lands:** existing API `YES`/`NO`/`NO_RESPONSE` remains; client maps `YES`/`NO_RESPONSE` → going, `NO` → not_going for queue logic. Out of play when every kid is not going. Copy uses **"going"** / **"not going"** (never ride-side **"drive"** language). Supersedes three-way RSVP UX on Agenda for this surface. Done (prior model): `[agenda-event-rsvp](specs/archive/agenda-event-rsvp.md)` |
...
| Focus card selection     | **Hero & coverage redesign (2026-08-28):** "Needs your attention" becomes a **swipeable carousel** over shared `getQueue` ([ADR-0001](decisions/ADR-0001-coverage-priority-rule.md)) — own-child unassigned rides **always** precede pending carpool requests (soonest-first within each tier); every slide independently actionable; empty queue → all caught up. Ranks: [`coverage-priority-engine`](specs/active/coverage-priority-engine.md) → [`hero-attention-carousel`](specs/planned/hero-attention-carousel.md) → [`weekly-list-focus-sync`](specs/planned/weekly-list-focus-sync.md). **Until carousel lands:** shipped single Focus + `selectFocusItem` remains live. Ride-side reverts are one-click, no dialog ([ADR-0002](decisions/ADR-0002-automatic-non-blocking-cancellation.md)). Unified status chip includes **You're driving · +N** when carpooling others — [`unified-ride-status-chip`](specs/planned/unified-ride-status-chip.md). **Agenda list section labels** (Done): **NEEDS YOUR ATTENTION** / **REST OF TODAY** / etc. — `[agenda-chip-section-headers](specs/archive/agenda-chip-section-headers.md)`. Prior single-hero rules: Done `[agenda-focus-next-action](specs/archive/agenda-focus-next-action.md)`, `[agenda-focus-carpool-actions](specs/archive/agenda-focus-carpool-actions.md)`. Rider initials chips: `[agenda-ride-rider-chips](specs/planned/agenda-ride-rider-chips.md)`. Solid Agenda↔ride id: Done `[calendar-item-event-key](specs/archive/calendar-item-event-key.md)`. |
...
| ---- | ------------------------------- | ------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 1    | coverage-priority-engine        | active  | 2026-08-28 · initial       | Shared `getQueue`, view-model types, derived helpers, and `autoDeclineUnofferable` — foundation for hero & coverage redesign (no UI) |
| 2    | household-driver-assignment     | planned | 2026-08-28 · initial       | DriverPicker: household chips + confirm, separated "Ask the team" path; household member confirm state                        |
| 3    | unified-ride-status-chip        | planned | 2026-08-28 · initial       | One glanceable status chip per game incl. **You're driving · +N**; separate CarpoolAskChip for inbound asks                   |
| 4    | hero-attention-carousel         | planned | 2026-08-28 · initial       | Swipeable "Needs your attention" carousel over `getQueue`; every slide actionable; empty → all caught up                      |
| 5    | weekly-list-focus-sync          | planned | 2026-08-28 · initial       | Collapsed-by-default agenda rows; highlight syncs to `getQueue[0]`; chronological list order                                    |
| 6    | ride-revert-undo                | planned | 2026-08-28 · initial       | One-click, no-dialog undos for every resolved ride state; **drive** vocabulary for ride-side copy                             |
| 7    | auto-decline-unofferable        | planned | 2026-08-28 · initial       | Wire auto-decline when own ride is `"requested"` — can't offer a ride you don't have                                        |
| 8    | attendance-manual-toggle        | planned | 2026-08-28 · initial       | Manual not-going toggle only; default going; never a hero item; **going** vocabulary                                         |
| 9    | carpool-pickup-detour           | planned | 2026-08-28 · initial       | Pickup town + color-coded detour minutes before Accept/Decline on carpool asks                                                |
| 10   | coverage-copy-a11y-polish       | planned | 2026-08-28 · initial       | Copy constants, layout separation, a11y/responsive polish pass across ranks 2–9                                               |
| 11   | agenda-ride-rider-chips         | planned | 2026-08-26 · enhancement   | Initials circles + first-name labels for who’s riding (reuse covering-avatar pattern); names only — no photos                 |
| 12   | auth-email-delivery             | planned | 2026-08-07 · enhancement   | Production SMTP/API mail for OTP — pre-beta gate (dev keeps log delivery); needed before real-device Expo auth                 |
| 13   | rn-expo-scaffold                | planned | 2026-08-25 · enhancement   | Expo app: OTP auth + shell + push-token registration hook against existing OpenAPI — infra for carpool push beta              |
| 14   | push-notifications              | planned | 2026-08-07 · initial       | Expo push for carpool ride **request / accept / deny (pass)** — pre-beta differentiator; not blocked on in-app inbox          |
| 15   | manual-event-team-link          | planned | 2026-08-13 · re-rank split | Manual events: attach to a team (feed UUID, carpool-eligible) or standalone (family tracking only) — web first                |
| 16   | carpool-recurring-rotation      | planned | 2026-08-16 · enhancement   | Standing teammate rotation for a recurring team event (e.g. every Tuesday); RSVP No drops that kid for that week only         |
| 17   | event-arrival-lead-time         | planned | 2026-08-11 · enhancement   | Editable arrival lead times: game ~30m / practice ~15m / other ~0 (defaults); refine leave-by target                          |
| 18   | coverage-leave-from             | planned | 2026-08-12 · enhancement   | Leave-from (and leave-by) per coverage when adults take separate cars/kids                                                    |
| 19   | conflict-travel-margin          | planned | 2026-08-12 · enhancement   | Soft “cutting it close” warn from leave-by/travel gaps (not hard overlap; after leave-from / lead-time)                       |
| 20   | calendar-conditional-get        | planned | 2026-08-12 · re-rank split | Server `ETag` + client `If-None-Match` / `304` on calendar background revalidate (after cheap list + client cache)            |
| 21   | event-venue-display-label       | planned | 2026-08-17 · enhancement   | Short venue label (rink, park, field) from geocoded event destination; Focus + Agenda fallback to full `location`             |
| 22   | web-auth-session-hardening      | planned | 2026-08-07 · enhancement   | HTTP-only cookie (or equivalent) for web — pre-beta gate; Expo stays Bearer                                                   |
| 23   | adult-optional-password         | planned | 2026-08-07 · re-rank split | Optional password for frequent users — pre-beta convenience (OTP remains primary)                                             |
| 24   | app-identity-rename             | planned | 2026-08-07 · initial       | Rename packages/clients from quickapp template identity before public beta                                                    |
...
| Id                            | Added                    | Summary                                                                                                                                                           |
| ----------------------------- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| member-profile-photos         | 2026-08-26 · enhancement | Real parent/teammate/kid images for card circles — after names/initials dogfood (`agenda-ride-rider-chips`); needs upload/storage product work                    |
...
| Id                            | Cancelled  | Reason |
| ----------------------------- | ---------- | ------ |
| assign-cancels-carpool-request | 2026-08-28 | Superseded by hero & coverage flow redesign — [`auto-decline-unofferable`](specs/planned/auto-decline-unofferable.md) + [`ride-revert-undo`](specs/planned/ride-revert-undo.md) |
...
| Id                       | Spec |
| ------------------------ | ---- |
| coverage-priority-engine | [active](specs/active/coverage-priority-engine.md) |
