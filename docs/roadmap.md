...
| Coverage                 | **Responsibility** rows (adult + kid subset + PENDING/CONFIRMED/DECLINED); any member assigns; assignee confirms/declines; kid exclusive per item on active rows; not a trip/seat plan. API `uncoveredKidIds` stays orthogonal to rides until later ranks wire lifecycle coupling. **Hero & coverage redesign (2026-08-28):** shared priority queue Done: [`coverage-priority-engine`](specs/archive/coverage-priority-engine.md); next [`auto-decline-unofferable`](specs/planned/auto-decline-unofferable.md) + [`ride-revert-undo`](specs/planned/ride-revert-undo.md) supersede `[assign-cancels-carpool-request](specs/planned/assign-cancels-carpool-request.md)` for ride/coverage coupling. Done: `[coverage-confirm-decline](specs/archive/coverage-confirm-decline.md)` |
...
| ---- | ------------------------------- | ------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 1    | weekly-list-focus-sync          | active  | 2026-08-28 · initial       | GameCard list rows; include queue items; highlight syncs to `getQueue[0]`; expanded gap → DriverPicker                          |
| 2    | ride-revert-undo                | planned | 2026-08-28 · initial       | One-click, no-dialog undos for every resolved ride state; **drive** vocabulary for ride-side copy                             |
| 3    | auto-decline-unofferable        | planned | 2026-08-28 · initial       | Wire auto-decline when own ride is `"requested"` — can't offer a ride you don't have                                        |
| 4    | attendance-manual-toggle        | planned | 2026-08-28 · initial       | Manual not-going toggle only; default going; never a hero item; **going** vocabulary                                         |
| 5    | carpool-pickup-detour           | planned | 2026-08-28 · initial       | Pickup town + color-coded detour minutes before Accept/Decline on carpool asks                                                |
| 6    | coverage-copy-a11y-polish       | planned | 2026-08-28 · initial       | Copy constants, layout separation, a11y/responsive polish pass across ranks 2–9                                               |
| 7    | agenda-ride-rider-chips         | planned | 2026-08-26 · enhancement   | Initials circles + first-name labels for who's riding (reuse covering-avatar pattern); names only — no photos                 |
| 8    | auth-email-delivery             | planned | 2026-08-07 · enhancement   | Production SMTP/API mail for OTP — pre-beta gate (dev keeps log delivery); needed before real-device Expo auth                 |
...
| 10   | push-notifications              | planned | 2026-08-07 · initial       | Expo push for carpool ride **request / accept / deny (pass)** — pre-beta differentiator; not blocked on in-app inbox          |
...
| 13   | event-arrival-lead-time         | planned | 2026-08-11 · enhancement   | Editable arrival lead times: game ~30m / practice ~15m / other ~0 (defaults); refine leave-by target                          |
...
| Id                     | Branch                   | Spec                                               |
| ---------------------- | ------------------------ | -------------------------------------------------- |
| weekly-list-focus-sync | `weekly-list-focus-sync` | [active](specs/active/weekly-list-focus-sync.md) |
