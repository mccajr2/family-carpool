...
Status: active  
Updated: 2026-08-17 (`/roadmap web-shell-page-header`)
...
**Clients (beta):** **Web + Android + iOS in parallel** for product verticals
(OpenAPI → backend → web + mobile together when a surface ships). **Visual /
restyle cluster is web-first**, then iOS/Android ports — mobile has lagged
the recent Agenda/Feeds design push.
...
| Web shell rail           | Signed-in web chrome is an **always-dark docked left rail** (independent of page theme): Calendar / Carpool / Family with icons; Settings Places / Garage / Feeds; **ACCOUNT** footer (avatar, email, role, sign out) **always visible** — pinned; nav list may scroll, Sign out must not. iOS/Android keep bottom tabs. Rail wordmark is placeholder chrome (accent mark) — do not lock copy; real name is `[app-identity-rename](specs/planned/app-identity-rename.md)`. Done: `[web-shell-nav-rail](specs/archive/web-shell-nav-rail.md)`. **Follow-up:** flush-left ~240px rail, fluid center (no destination `Card` border), Calendar-only empty right rail — `[web-shell-page-frame](specs/active/web-shell-page-frame.md)`. After that, destination header type/copy (token scale, not mock 34px) — `[web-shell-page-header](specs/planned/web-shell-page-header.md)` |
...
| 2    | web-shell-page-header              | planned | 2026-08-17 · enhancement                                         | Web destination header: token type + ink/slate; Calendar Today/date subtitle; no new 34px/36/44 roles                                                           |
| 3    | agenda-focus-card-polish           | planned | 2026-08-17 · enhancement                                         | Web Focus card: larger isolated ring, covering under ring, overlap chip; same handlers                                                                          |
| 4    | agenda-list-chips                  | planned | 2026-08-17 · enhancement                                         | Web kid-filter + collapsed row status as chips/avatars (no “3 STOPS” until multi-stop)                                                                          |
| 5    | feeds-page-redesign                | planned | 2026-08-15 · re-rank split                                       | Web Feeds cards from Claude dark mock; OWNED/NO CARPOOL chips; zero behavior change                                                                             |
| 6    | agenda-week-glance                 | planned | 2026-08-17 · enhancement                                         | Web five-day coverage/driver strip; not the month grid; no carpool stop card                                                                                    |
| 7    | agenda-focus-card-mobile           | planned | 2026-08-14 · enhancement                                         | iOS + Android port of the polished Focus card (after web `agenda-focus-card-polish`)                                                                            |
| 8    | agenda-full-page-redesign-mobile   | planned | 2026-08-15 · enhancement                                         | iOS + Android day-grouped rows including list chips (after web `agenda-list-chips`)                                                                             |
| 9    | feeds-page-redesign-mobile         | planned | 2026-08-15 · enhancement                                         | iOS + Android port of Feeds card restyle (after web `feeds-page-redesign`)                                                                                      |
| 10   | agenda-week-glance-mobile          | planned | 2026-08-17 · enhancement                                         | iOS + Android five-day status strip (after web `agenda-week-glance`)                                                                                            |
...
| 12   | manual-event-team-link             | planned | 2026-08-13 · re-rank split                                       | Manual events: attach to a team (feed UUID, carpool-eligible) or standalone (family tracking only)                                                              |
...
| 18   | family-calendar-grid               | planned | 2026-08-10 · re-rank split                                       | iOS-style **month/week calendar grid** on top of the unified schedule (after Agenda UX is solid on web + mobile)                                                |
| 19   | ui-system-destination-adoption     | planned | 2026-08-10 · note carried from cross-platform-ui-system approval | Remainder token adoption (Carpool / Family / Places / Garage / grid) — Agenda/Feeds redesigns already adopt; re-run screenshots + WCAG AA per leftover surface |
| 20   | typography-font-family             | planned | 2026-08-14 · enhancement                                         | iOS + Android font pairing (asset bundling); web shipped separately as `typography-web`                                                                         |
| 21   | auth-email-delivery                | planned | 2026-08-07 · enhancement                                         | Production SMTP/API mail for OTP — pre-beta gate for real users (dev keeps log delivery)                                                                        |
| 22   | web-auth-session-hardening         | planned | 2026-08-07 · enhancement                                         | HTTP-only cookie (or equivalent) for web — pre-beta gate; mobile stays Bearer                                                                                   |
...
| in-app-notifications        | 2026-08-16 · enhancement | In-app alert inbox (coverage asks, ride request/accept, rotation gaps); promote before `carpool-driver-gap-fill` — not part of Agenda/Feeds redesign              |
| push-notifications          | 2026-08-07 · initial     | Native push (APNs/FCM) for the same events as `in-app-notifications`; stays parking until the in-app inbox exists                                                 |
...
| ios-auth-unreachable-parity | 2026-08-16 · enhancement | iOS: unreachable `/api/auth/me` must keep the session and show Retry, not bounce to sign-in (web + Android already do)                                            |
