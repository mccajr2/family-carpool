...
Status: active  
Updated: 2026-08-16 (`/pr agenda-full-page-redesign`)
...
| Visual language          | **One visual priority per screen**; everything else calmer. Every color pairing **WCAG AA** before use (4.5:1 text, 3.0:1 icons) — don't eyeball. Destructive actions (Remove/Delete) get **less** weight than neutral (Edit/Sync). A design pass restyles only — **same handlers**. Web typography: Space Grotesk (display) + Plus Jakarta Sans (body) — `[typography-web](specs/active/typography-web.md)`. Mobile stays `system-ui` until `[typography-font-family](specs/planned/typography-font-family.md)` (asset bundling). Token roles in `design-tokens/tokens.json` are the contract; `hero*` color roles are Focus-card urgent spotlight only. |
...
| ---- | ---------------------------------- | ------- | ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | typography-web                     | active  | 2026-08-16 · re-rank split                                       | Space Grotesk (display) + Plus Jakarta Sans (body), web only. Mobile stays deferred (asset bundling).                                                           |
...
| Id | Branch | Spec |
| -- | ------ | ---- |
| typography-web | `typography-web` | [active](specs/active/typography-web.md) |
