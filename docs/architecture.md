...
| Stop-sequence optimize | Pure leaveby helper over waypoints (fixed start, reorderable middles, fixed end); minimize total pairwise driving duration; brute-force ≤ 7 middles else nearest-neighbor; missing duration → soft-fail (no invented minutes). **Auto on itinerary build / fingerprint refresh** when 2+ geocodable middles. Optional driving-adult middle reorder (`PUT`) keeps the same fingerprint until the next rebuild re-auto-optimizes. **Wired today:** single-event TO Route (home → ACCEPTED teammate pickups → venue). **Later callers:** day-block multi-event stop lists and per-leg FROM assembly ([`day-block-route`](specs/active/day-block-route.md)) — same helper, different waypoint assembly |
