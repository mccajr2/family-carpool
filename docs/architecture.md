...
**family circle + kids + named places** (and team carpool rides).
...
2. **Any-member household content** — day-to-day shared facts everyone may
   ...
   circle) are any space-member adult — **Accept** is empty-body; any
   space-member adult from another circle (own-circle → **409**; not
   `PENDING` → **409**; member/unknown → **404**; no `drives` / vehicle /
   seat checks).
...
even when unlocated), `FamilyGeocodeApi`. Feeds public surface used by carpool:
...
- **Coverage assignment** belongs to a calendar item (`MANUAL`|`FEED` + item
  ...
  on Agenda. Not a vehicle/trip plan — **RideRequest** (below) is the trip
  seat loop. Vehicle capacity (if revived) → parking `garage-capacity`.
- **RSVP** belongs to a calendar item + kid (`YES`|`NO`; missing ≡ `NO_RESPONSE`);
  circle-visible; attendance is separate from coverage responsibility. On a
  carpool-eligible **FEED** event, RSVP **`NO`** (not going) also removes that
  kid from the circle’s active ride plan and clears **all** transport legs
  (pending asks, waiting household, confirmed); last going kid cleared → cancel
  the plan (same automatic-cancel spirit as ADR-0002 / coverage hard-release).
- **RideRequest** belongs to a **CarpoolSpace** + feed **eventKey**
  ...
  unchanged. Status `PENDING` \| `ACCEPTED` \| `CANCELLED`. Each plan always
  has two leg slots — **TO** (Getting there) and **FROM** (Coming back) — with
  phase `NEEDS_RIDE` \| `WAITING_HOUSEHOLD` \| `ASKED_TEAM` \| `CONFIRMED`
  (Accept = confirm; no claimed-unconfirmed state). Create defaults to
  **both** legs; optional `legs: [TO]` / `[FROM]` for a single-leg ask.
  **Accept** (empty body): space-member adult from another circle → sets
  `ACCEPTED`, records accepting adult/circle, confirms asked open legs,
  clears passes, sets RSVP YES for kids on that ride; **no** vehicle recorded;
  **no** seat/vehicle/`drives` checks. Own-circle → **409**; not `PENDING` →
  **409**; member/unknown → **404**. Coverage does not occupy seats. Combined
  Cancel / Withdraw when both legs share the same assignee; else per-leg
  `legs` body required (**409** if omitted when assignees differ).
...
as needed). Until then: no capacity chrome in Settings, no public garage API,
no external vehicle-catalog integration as current product behavior.
...
| Rides | Space-scoped **RideRequest** on upcoming **feed** events (same local today → +30d spirit as Agenda). Household-only **Save ride plan** may also persist circle-local `PLAN`s with **null `space_id`** before Enable carpool (`GET/POST /api/carpool/ride-plans`); Ask the team still requires a space. Enable attaches **all** matching null-space PLANs for that event onto the new space (not a single row). `defaultKidIds` / create default = feed-linked kids who are **not RSVP NO** (YES and No response) and not already on this circle’s ACCEPTED ride (subset override OK). **Create does not change RSVP** (requested kids may stay No response). Each TO/FROM leg stores one **family-side place** (Default / named located circle place / one-time free-text) and, on Ask legs, a **meet side** (`REQUESTER` = meet at the requesting circle’s place; `ACCEPTOR` = meet at the accepting driver’s place, bound on Accept from the accepter’s resolved leave-from — membership default, else first located by name; Accept **400** if unresolved). **Default** place resolves at write/serve as the signed-in adult’s **My default leave-from** when set and usable, else first located place by name (same fallback as leave-by); both place id and one-time address null on the leg. Ride-level `pickupPlaceName` / `pickupAddress` are derived from the **TO** family-side place when TO meet side is `REQUESTER`; when TO is `ACCEPTOR` and still pending, pickup is the display label “Driver’s place” without a street address, then the bound accepter place after Accept. Ask with TO `REQUESTER` and no resolvable TO pickup → **400**; TO `ACCEPTOR` may post without a requester place. Route pickup stops / inbound PickupLine + detour run only when TO meet side is `REQUESTER` (no requester-house pickup stop for TO `ACCEPTOR`). **CalendarRoute** (Route tab) orders that TO pickup sequence via leaveby’s reusable stop-sequence optimizer (see Leave-by); Route stays TO-only until day-block There/Back chrome. Each plan has **TO** + **FROM** leg slots (`NEEDS_RIDE` \| `WAITING_HOUSEHOLD` \| `ASKED_TEAM` \| `CONFIRMED`); create defaults to **both** legs (optional single-leg `legs`). List/detail return `legs` / `ownLegs` when there is exactly one active own plan; expose **`ownRequests` (or equivalent)** for all of this circle’s active plans — singular fields are `null` when 0 or 2+. **Independent per-leg drivers** (in / `carpool-leg-split-plans`): web shared split editor (**Different plans for each leg** → Getting there / Coming back) persists distinct household vs Ask outcomes onto TO/FROM via **Save ride plan**. **Independent per-leg family-side places** (in / `carpool-leg-places`): **Picking up from** on TO and **Dropping off at** on FROM when that leg is household or Ask with meet **Our place** (`REQUESTER`); omit for **Needs ride** or Ask with **Driver’s place** (`ACCEPTOR`). **Meet side** (in / `carpool-meet-at`): Ask legs choose **Our place** / **Driver’s place** (default Our place) on simple + split + kid-split DriverPicker. Simple round-trip Confirm / Post / Save still uses one Leave from applied to Our-place / household legs. **Per-kid distinct plans** (in / `carpool-kid-split-plans`): after kids diverge, persist **multiple active RideRequests per circle+event**, grouped by identical TO/FROM driver outcome, family-side place, **and** meet side (same phase + assignee adult / circle + place triad + meet side per leg). Invariant: a kid appears on **at most one** non-cancelled plan for `(requesting circle, event)` — replace one-plan-per-event unique indexes; Save is an atomic replace of this circle’s active plans for the event (submitted groups cover every going kid; `NEEDS_RIDE` on a leg allowed). Matching plans still share one multi-seat PENDING ask (v1 “one request covering attending kids” when outcomes match). Partial accept of a multi-kid bag stays out — splitting asks when kids diverge avoids it. Hero / Focus `getQueue` treats any going kid with an in-play `NEEDS_RIDE` (or blank plan) as a gap even when a sibling is asked or confirmed, treats any in-play `ownLegs` leg in `NEEDS_RIDE` as an own-ride gap even when the other leg is asked or confirmed, and treats `WAITING_HOUSEHOLD` assigned to the signed-in adult as pending-confirm-for-self (`confirm-household` / `decline-household` / `clear-legs` flip only those plan(s), not every sibling). **Accept** (empty body): any space-member adult from another circle; confirms asked open legs (both when round-trip ask); for each ASKED_TEAM leg with meet side `ACCEPTOR`, binds that leg’s place from the accepter leave-from (Accept **400** if unresolved); own-circle → **409**; not `PENDING` → **409**; member/unknown → **404**; **no** `403` for `drives=false`; **no** seat/vehicle checks; success sets `ACCEPTED` + accepting adult/circle, clears passes, sets **RSVP YES** for kids on that ride (requesting circle); **no** vehicle recorded. Cancel / Withdraw / Pass do not auto-change RSVP. **Pass**: per-adult decline of a `PENDING` ask (request stays `PENDING` for others; list marks `passedByMe`; cleared on Accept/Cancel). Cancel (requesting) / Withdraw (accepting): combined when assignees match; else require per-leg `legs` (**409** if omitted when they differ). **RSVP NO** on a FEED event removes that kid from the circle’s active plan(s) / clears all their legs including `WAITING_HOUSEHOLD`; empty plan → cancel — siblings’ other plans stay. Coverage orthogonal. No ride fields on `CalendarItem` |
| Clients | **Web Calendar / Agenda is the primary ride surface** — carpool-eligible FEED rows Request / status; Focus CTA precedence Confirm → Accept/Pass → **Request** (Assign secondary if remaining coverage gap) → Assign → calm (family decisions beat ride Accepts in Today/Tomorrow ranking). **Calendar is self-sufficient for reverse actions when those states are shown** (Focus is not duplicated in the day list): outline **Cancel** for own `PENDING`/`ACCEPTED` on Focus; outline **Withdraw** for accepted-by-us on Focus and on expanded rows. Expanded rows keep Request/Cancel for own request — **no** Accept/Pass on rows. Collapsed Focus / Agenda (+ Carpool ride lines) show **dual leg chips** (`Getting there:` / `Coming back:`) from plan legs when an own request exists (or legs are non-blank); when own plans **diverge**, one chip group per distinct plan (kid first names joined) with matching-leg collapse inside the group — shared chrome (no per-kid prefix) while plans still match. Household coverage Confirm/Assign chrome stays coverage-driven until the coverage card unifies writes — except when a confirmed household driver also accepted inbound asks, chips overlay inbound confirmed legs onto Getting there / Coming back (`You're driving · +n` per leg). Inbound Accept/Pass (Focus + expanded inbound rows) shows which leg(s) are asked. **DriverPicker** (Focus / hero + expanded uncovered own-ride): default round-trip chrome with Ask **Meet where?** (Our place / Driver’s place) and one **Leave from** when any Ask leg is Our place (or household); **Different plans for each leg.** (shared-for-all-kids; **Meet where?** under each Ask leg; **Picking up from** / **Dropping off at** when Our place / household); **Different plans for each kid.** when 2+ going kids (nested per-kid sections + nested leg-split; per-kid meet side + Leave from / per-leg places; atomic Save with identical-outcome grouping including places and meet sides; Back restores simple without merge — Confirm / Post from simple still writes one shared plan). **Ride-line density** matches Carpool tab fields (shared `carpoolDisplay` helpers): Focus Accept/Pass incoming = requesting circle · kid first names · **seats** · pickup (`place, address`) + leg chips; Focus + expanded own request = Calendar status (`Requested` / `Riding with {circle}`) · kids · seats · pickup; Focus + expanded accepted-by-us = requesting circle · kids · seats · pickup. **ACCEPTED own ride is not a Calendar coverage gap** in chrome: Needs coverage / Assign / Focus family-decision use remaining gap kids (`uncoveredKidIds` minus ACCEPTED kids on this circle’s own plan(s)); API `uncoveredKidIds` stays orthogonal. **Carpool tab** remains membership + secondary ride list (**Pass** on PENDING other asks when `!passedByMe`; Accept-after-Pass kept; no un-pass; same requestable-kid rules; may keep “Accepted by {circle}”). **Feeds** (Organizer) keeps per-feed Enable / Request / Owned chrome. Caregivers never gain Feeds manage. Android/iOS Agenda/Focus ride UI is parked (`carpool-request-accept-mobile`, `agenda-focus-card-mobile`); KMP `sharedLogic` is frozen — do not update for OpenAPI leg / multi-plan fields |
| Out of scope | Partial accept of a multi-kid ask; merge two families onto one vehicle (multi-stop); Ask-team **radius / distance matching** of candidate drivers (parked; no id yet); un-pass; least-privilege nanny roster → parked `carpool-least-privilege`; vehicle capacity → parking `garage-capacity`. (Per-leg **drivers** + hero per-leg gaps + matching-chip collapse + PLAN/`clear-legs` cancel are in-scope under Rides / `carpool-leg-split-plans`; **per-kid distinct plans** + multi-request grouping + per-kid hero gaps / chip groups are in-scope under Rides / `carpool-kid-split-plans`; **per-leg family-side places** (Picking up from / Dropping off at; Default / named / one-time) are in-scope under Rides / `carpool-leg-places`; **Ask meet side** (Our place / Driver’s place; Accept bind; no requester pickup for TO Driver’s place) is in-scope under Rides / `carpool-meet-at`; default DriverPicker chrome shipped in `carpool-ride-coverage-card`. Archive note: `carpool-leg-chip-collapse` display collapse shipped with dogfood batch 2.) |
...
| Multi-stop itinerary | `GET/PUT …/calendar/{source}/{itemId}/route?leg=TO\|FROM` (`CalendarRoute`, default `leg=TO`): ordered stops + `legMinutes` + leave-by for a routable ride. Resolves the combined driving-block for `(adult, leg, item)`; persists under a **member-set** key `(adultId, leg, ordered member source+itemId)` + stop fingerprint (same itinerary via any member `itemId`). Soft-fail like leave-by (`OK` \| `UNAVAILABLE`). Reuses `geocode_cache` + `leaveby_route_cache` — no second duration cache. Optional response echo: `leg`, `memberItemIds`, and the per-leg itinerary **home-side** triad (`leaveFromPlaceId` / `leaveFromPlaceName` / `leaveFromAddress`). **Home-side override** (Leaving from on TO / Returning to on FROM): `PUT …/route/origin?leg=` with the same body shape as Agenda leave-from (Default / named located place / one-time). Stored on the member-set itinerary row only — **does not** write calendar-item or coverage leave-from; TO and FROM overrides are independent. Resolution: override when set, else combined → membership default leave-from, singleton → path-item leave-from chain (FROM end → driver leave-from / home). Middles that match the governing HOME (placeId / address / geocode) are dropped before optimize so HOME appears once |
| Stop-sequence optimize | Pure leaveby helper over waypoints (fixed start, reorderable middles, fixed end); minimize total pairwise driving duration; brute-force ≤ 7 middles else nearest-neighbor; missing duration → soft-fail (no invented minutes). **Auto on itinerary build / fingerprint refresh** when 2+ geocodable middles (including after a home-side origin change). Optional driving-adult middle reorder (`PUT` + `leg`) keeps the same fingerprint until the next rebuild re-auto-optimizes. **Callers:** TO (home → household/teammate pickups → venue) and FROM (venue → dropoffs → home) for singleton or combined day-block member sets ([`day-block-route`](specs/archive/day-block-route.md)) |
...
| Out of scope | Travel/leave-by soft warn → `conflict-travel-margin`; seats / vehicles → parking `garage-capacity`; trip seat math → Carpool **RideRequest** (when capacity revives) |
...
│       ├── leaveby/      # Leave-from + OSRM estimate + route cache + stop-sequence optimize
