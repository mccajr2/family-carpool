...
| Rides | Space-scoped **RideRequest** on upcoming **feed** events (same local today → +30d spirit as Agenda). Household-only **Save ride plan** may also persist a circle-local `PLAN` with **null `space_id`** before Enable carpool (`GET/POST /api/carpool/ride-plans`); Ask the team still requires a space. Enable attaches matching null-space PLANs onto the new space. `defaultKidIds` / create default = feed-linked kids who are **not RSVP NO** (YES and No response) and not already on this circle’s ACCEPTED ride (subset override OK). **Create does not change RSVP** (requested kids may stay No response). Pickup = requester house address snapshot (ask). Each plan has **TO** + **FROM** leg slots (`NEEDS_RIDE` \| `WAITING_HOUSEHOLD` \| `ASKED_TEAM` \| `CONFIRMED`); create defaults to **both** legs (optional single-leg `legs`). List/detail return `legs` / `ownLegs`. **Independent per-leg drivers** (in): web split editor (**Different plans for each leg** → Getting there / Coming back) persists distinct household vs Ask outcomes onto TO/FROM via **Save ride plan** (atomic or short compose of existing writes); leave-from stays ride/coverage-level (one shared combobox when any selected leg is household) — **not** per-leg place columns. Hero / Focus `getQueue` treats any in-play `ownLegs` leg in `NEEDS_RIDE` as an own-ride gap even when the other leg is asked or confirmed, and treats `WAITING_HOUSEHOLD` assigned to the signed-in adult as pending-confirm-for-self (`confirm-household` / `decline-household` flip only those legs). **Accept** (empty body): any space-member adult from another circle; confirms asked open legs (both when round-trip ask); own-circle → **409**; not `PENDING` → **409**; member/unknown → **404**; **no** `403` for `drives=false`; **no** seat/vehicle checks; success sets `ACCEPTED` + accepting adult/circle, clears passes, sets **RSVP YES** for kids on that ride (requesting circle); **no** vehicle recorded. Cancel / Withdraw / Pass do not auto-change RSVP. **Pass**: per-adult decline of a `PENDING` ask (request stays `PENDING` for others; list marks `passedByMe`; cleared on Accept/Cancel). Cancel (requesting) / Withdraw (accepting): combined when assignees match; else require per-leg `legs` (**409** if omitted when they differ). **RSVP NO** on a FEED event removes that kid from the circle’s active plan / clears all their legs including `WAITING_HOUSEHOLD`; empty plan → cancel. Coverage orthogonal. No ride fields on `CalendarItem` |
| Clients | **Web Calendar / Agenda is the primary ride surface** — carpool-eligible FEED rows Request / status; Focus CTA precedence Confirm → Accept/Pass → **Request** (Assign secondary if remaining coverage gap) → Assign → calm (family decisions beat ride Accepts in Today/Tomorrow ranking). **Calendar is self-sufficient for reverse actions when those states are shown** (Focus is not duplicated in the day list): outline **Cancel** for own `PENDING`/`ACCEPTED` on Focus; outline **Withdraw** for accepted-by-us on Focus and on expanded rows. Expanded rows keep Request/Cancel for own request — **no** Accept/Pass on rows. Collapsed Focus / Agenda (+ Carpool ride lines) show **dual leg chips** (`Getting there:` / `Coming back:`) from plan legs when an own request exists (or legs are non-blank); household coverage Confirm/Assign chrome stays coverage-driven until the coverage card unifies writes — except when a confirmed household driver also accepted inbound asks, chips overlay inbound confirmed legs onto Getting there / Coming back (`You're driving · +n` per leg). Inbound Accept/Pass (Focus + expanded inbound rows) shows which leg(s) are asked. **Ride-line density** matches Carpool tab fields (shared `carpoolDisplay` helpers): Focus Accept/Pass incoming = requesting circle · kid first names · **seats** · pickup (`place, address`) + leg chips; Focus + expanded own request = Calendar status (`Requested` / `Riding with {circle}`) · kids · seats · pickup; Focus + expanded accepted-by-us = requesting circle · kids · seats · pickup. **ACCEPTED own ride is not a Calendar coverage gap** in chrome: Needs coverage / Assign / Focus family-decision use remaining gap kids (`uncoveredKidIds` minus ACCEPTED `ownRequest.kidIds`); API `uncoveredKidIds` stays orthogonal. **Carpool tab** remains membership + secondary ride list (**Pass** on PENDING other asks when `!passedByMe`; Accept-after-Pass kept; no un-pass; same requestable-kid rules; may keep “Accepted by {circle}”). **Feeds** (Organizer) keeps per-feed Enable / Request / Owned chrome. Caregivers never gain Feeds manage. Android/iOS Agenda/Focus ride UI is parked (`carpool-request-accept-mobile`, `agenda-focus-card-mobile`); KMP `sharedLogic` is frozen — do not update for OpenAPI leg fields |
| Out of scope | Partial accept of a multi-kid ask; merge two families onto one vehicle (multi-stop); per-kid distinct plans → `carpool-kid-split-plans`; **true per-leg locations** / Ask-team meet-at sub-flow (independent Picking up from / Dropping off at on each leg) → `carpool-meet-at`; un-pass; least-privilege nanny roster → parked `carpool-least-privilege`; vehicle capacity → parking `garage-capacity`. (Per-leg **drivers** + shared leave-from + hero per-leg gaps + matching-chip collapse + PLAN/`clear-legs` cancel are in-scope under Rides / `carpool-leg-split-plans`; default DriverPicker chrome shipped in `carpool-ride-coverage-card`. Archive note: `carpool-leg-chip-collapse` display collapse shipped with dogfood batch 2.) |
