...
`carpool-request-accept` + `carpool-leg-to-from`:
...
| Carpool | Opt-in **team space** keyed **one per normalized feed URL**. Membership is the **circle**. Organizer **Enable** owns; join by **code** (`ensureFeed` if URL missing) or **request** (owner admit/decline, in-app). **Needs** (`CarpoolRequest`: one row per kid × event × circle; `legsNeeded` = `{TO}` / `{FROM}` / `{TO,FROM}`; roll-up `UNCOVERED` \| `PARTIAL` \| `FULLY_COVERED`) are separate from **fulfillments** (`Ride`: one row per driver × leg × event; passengers = request ids). Members see kid first names, seats, and pickup address. HTTP `/api/carpool/*`. See Team carpool space below |
...
+ [`carpool-leg-to-from`](specs/archive/carpool-leg-to-from.md):
...
| Rides | **Need ≠ fulfillment.** Space-scoped **`CarpoolRequest`** (one row per kid × event × circle; uniqueness → **409** on duplicate; PATCH `legsNeeded` to change) + **`Ride`** (one active row per driver × `TO`\|`FROM` × event) on upcoming **feed** events (same local today → +30d spirit as Agenda). `legsNeeded` persists only `{TO}` / `{FROM}` / `{TO,FROM}` — client may send `TO`/`FROM`/`BOTH`; default create = round trip. Per leg: `OPEN` \| `CONFIRMED`; request roll-up `UNCOVERED` \| `PARTIAL` \| `FULLY_COVERED`. `defaultKidIds` / create default = feed-linked kids who are **not RSVP NO** and not already `FULLY_COVERED` for this circle on the event. **Create request does not change RSVP**. Pickup = requester house address snapshot (`meetPoint*` null placeholders only). **Accept** (inbound): other circle, `drives=true`, chosen vehicle — creates one `Ride` per still-**OPEN** leg (round trip → two rows when both OPEN); passengers = request ids from **one** requesting circle; seat math per leg (`seats − 1 − driver-circle YES kids already on this Ride − passengers`); **409** if that request leg is already a passenger on another active Ride or the vehicle is already committed for `(event, leg)`; **successful Accept sets RSVP YES** for kids on those requests. Household DriverPicker uses the same create-`Ride` path (self / other household adult / teammate). **Pass**: per-adult soft decline of an **OPEN**-leg inbound **request** (not per Ride; list marks `passedByMe`). **Cancel** (requesting) / **Withdraw** (driving) target a **specific `Ride` id** (one leg); the other leg is unchanged. Cancel / Withdraw / Pass do not auto-change RSVP. Coverage orthogonal. No ride fields on `CalendarItem`. HTTP: `/ride-requests` (create/PATCH/pass/accept) + `/rides` (list/create/cancel/withdraw) |
| Clients | **Web Calendar / Agenda is the primary ride surface** — carpool-eligible FEED rows Request (per-kid **To / From / Round trip**, default Round trip) / status; Focus CTA precedence Confirm → Accept/Pass → **Request** (Assign secondary if remaining coverage gap) → Assign → calm (family decisions beat ride Accepts in Today/Tomorrow ranking). **Calendar is self-sufficient for reverse actions when those states are shown**: outline **Cancel** / **Withdraw** per active `Ride` (one leg) on Focus + expanded rows; **Accept covers all still-OPEN legs** (no Accept leg picker). Expanded inbound rows (`AgendaInboundRequestRow`) keep Accept/Pass/Withdraw; own rows keep Request/Cancel + DriverPicker. **Ride-line density** matches Carpool tab via `carpoolDisplay` — show **per-leg** fulfillment when TO/FROM differ (e.g. TO confirmed, FROM open). Status chips (`rideStatusChip`): partial round-trip copy when roll-up is `PARTIAL`; gap / queue treat `PARTIAL` as still needing a decision (leave queue / clear remaining gap only on `FULLY_COVERED`). **Route** (`canRoute`): true iff a confirmed **TO** Ride covers that kid (FROM-only does not). **`FULLY_COVERED` own need is not a Calendar coverage gap** in chrome: Needs coverage / Assign / Focus family-decision / week-glance use remaining gap kids (`uncoveredKidIds` minus kids whose own need is `FULLY_COVERED`); `PARTIAL` still counts as needs coverage; API `uncoveredKidIds` stays orthogonal. Status copy **Riding with {driving circle}** when teammate-covered. **Carpool tab** remains membership + secondary ride list (Pass when `!passedByMe`; Accept-after-Pass kept; no un-pass; same requestable-kid + legs control). **Feeds** (Organizer) keeps per-feed Enable / Request / Owned chrome. Caregivers never gain Feeds manage. Android/iOS Agenda/Focus ride UI is parked (`carpool-request-accept-mobile`, `agenda-focus-card-mobile`); KMP `sharedLogic` OpenAPI updates frozen |
| Out of scope | Accept-subset-of-legs picker / split-plans disclosure UI → `carpool-leg-split-plans`; merge two families’ passengers onto one vehicle → `carpool-multi-family-ride`; meet-at / early-late / FROM Route tab; un-pass; least-privilege nanny roster → parked `carpool-least-privilege`; vehicles live in the **family garage** |
