...
| Stop-sequence optimize | Pure leaveby helper over waypoints (fixed start, reorderable middles, fixed end); minimize total pairwise driving duration; brute-force ≤ 7 middles else nearest-neighbor; missing duration → soft-fail (no invented minutes). **Auto on itinerary build / fingerprint refresh** when 2+ geocodable middles. Optional driving-adult middle reorder (`PUT` + `leg`) keeps the same fingerprint until the next rebuild re-auto-optimizes. **Callers:** TO (home → household/teammate pickups → venue) and FROM (venue → dropoffs → home) for singleton or combined day-block member sets ([`day-block-route`](specs/active/day-block-route.md)) |
